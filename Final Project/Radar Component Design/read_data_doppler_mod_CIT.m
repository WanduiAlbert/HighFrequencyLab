...
%Modified 11.29.2016 by D.Russell
...
fc = 5.89E9; %(Hz) Enter your CW frequency here

N = Tp*FS; %# of samples per pulse

%the input appears to be inverted
s = -1*Y(:,2);
Yraw=Y; %save for time series
clear Y;
...
%plot
imagesc(velocity*2.23694,time,v-mmax,[-35, 0]);
colorbar;
xlim([0 100]); %limit velocity axis
xlabel('Velocity (miles/hour)');
ylabel('time (sec)');

figure
imagesc(velocity,time,v-mmax,[-35, 0]);
colorbar;
xlim([0 50]); %limit velocity axis
xlabel('Velocity (m/s)');
ylabel('time (sec)');

figure
plot(Yraw(3e4:end,2));
xlabel('Sample Number');ylabel('Volts');title('Sound Card Input');
