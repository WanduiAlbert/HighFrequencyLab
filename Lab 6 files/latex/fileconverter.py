...
def loaddata(filename):
    datatype = [("freq", np.float64),   ("S11-mag", np.float64),  ("S11-arg", np.float64),\
       ("S21-mag", np.float64), ("S21-arg", np.float64),   ("S12-mag", np.float64),\
       ("S12-arg", np.float64),   ("S22-mag", np.float64),  ("S22-arg", np.float64)]
...
def parameter_converter(freq, S11, S12, S21, S22):
    """
    Converts from 2 port to 3 port parameters for a transistor. 
    Returns the data as a list that can be used to construct a structured array
    """
    eps = S11 + S12 + S21 + S22
    D = 4-eps
    D11 = 1 - S11 - S12
...
def convertdata(data):
    ...
    final_dtype = [("freq", np.float64),   ("S11", np.complex128),\
        ("S12", np.complex128), ("S13", np.complex128),\
       ("S21", np.complex128),   ("S22", np.complex128), ("S23", np.complex128),\
       ("S31", np.complex128), ("S32", np.complex128), ("S33", np.complex128)] 
       # data type for the intermediate files
...
def savedata(final_data, save_dir, save_name, initial_file):
    ...
    with open(save_dir + save_name, 'w') as save_f:
        ...
        params = "!  Freq-GHz   S11-mag  S11-arg   S12-mag  S12-arg   S13-mag  S13-arg"
        params += "   S21-mag  S21-arg   S22-mag  S22-arg   S23-mag  S23-arg"
        params += "   S31-mag  S31-arg   S32-mag  S32-arg   S33-mag  S33-arg\n"
        save_f.write(params)

        for i in range(N):
            save_f.write("{0: 6.3f}  {1: 6.3f}  {2: 6.3f}  {3: 6.3f}  {4: 6.3f}\
              {5: 6.3f}  {6: 6.3f}\n".format(save_data[i, 0], save_data[i, 1],\
             save_data[i, 2], save_data[i, 3], save_data[i, 4], save_data[i, 5], save_data[i, 6]))

            save_f.write("      {0: 6.3f}  {1: 6.3f}  {2: 6.3f}  {3: 6.3f}\
